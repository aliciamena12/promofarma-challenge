import ProductTile, { Product } from "./product-tile.component";
...
const mockProduct: Product = {
  ...
  describe('product-tile component: ', () => {
    ...
    it('is rendered', () => {
      //given
      const { getByText, getByTitle, getByAltText, container } = render(
        <ProductTile productData={mockProduct} />
      );

      //then
      expect(getByAltText('Image of product: Ibuprofeno').getAttribute('alt')).toEqual("Image of product: " + mockProduct.name);
      expect(getByText('Ibuprofeno').innerHTML).toEqual(mockProduct.name);
      expect(getByText('20 units · pills').innerHTML).toEqual(mockProduct.packagingSize + " · " + mockProduct.dosageForm);
      expect(getByText('PillCompany').innerHTML).toEqual(mockProduct.supplier);
      expect(getByText('6,50 €').innerHTML).toEqual(mockProduct.prices.salesPrice.formattedValue);
      expect(getByText('8,25 €').innerHTML).toEqual(mockProduct.prices.recommendedRetailPrice.formattedValue);
      expect(getByText('1 €/St.').innerHTML).toEqual(mockProduct.baseprice);
